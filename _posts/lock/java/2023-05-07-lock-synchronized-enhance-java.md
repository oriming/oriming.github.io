...
<font color=#999AAA >`synchronized`关键字往往是java中在处理高并发时的首选，尤其是**JDK6**之后，对synchronized不断的优化，提供了`三种锁的实现`，包括 `偏向锁 ⋙ 轻量级锁 ⋙ 重量级锁`，还提供了**自动的升级和降级机制**，使synchronized的性能得到极大的提升。
秉承废话不多说的规则，咱们开始...
</font>
